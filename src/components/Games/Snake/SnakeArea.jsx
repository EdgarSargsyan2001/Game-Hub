...
function SnakeArea (){
    ...
    const [localScore,setLocalScore] = useState(localStorage.getItem("score"))
    ...
    const moveFunc = ()=> {
        ...
        if(head[0] === food[0] && head[1] === food[1]){
            ...
            getNewFood();
             ...
             if (localStorage.getItem("score") != null && Number(localStorage.getItem("score"))<=snake.length ){
   ...
   if(gameOver && snakeMoveSpeed){
    return(
    <div className="area1">
        <div className="score2">Record Score: {localScore}</div>
       <div className="area">
            ...
            {cellsValue.map((row,indexR)=>{
                ...
                <div key={indexR} className="row">
                    {row.map((cell,indexC)=>{
                        ...
                        return(
                         <div className={`cell ${type}`} type={type}  key={indexC}></div>
                         )
                    })}
                </div>
                )
...
else if (snakeMoveSpeed )return (
        <div className="area">
            <div className="score1">Record Score: {localScore}</div>
            <p style={{
                fontSize:"70px",
                fontWeight:"bold",
                alignItems: "center",
